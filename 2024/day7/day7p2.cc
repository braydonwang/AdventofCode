...
bool valid(ll acc, int ind, ll target, vector<ll> &nums) {
    ...
    if (valid(acc + nums[ind], ind+1, target, nums) || valid(acc * nums[ind], ind+1, target, nums)) {
        return true;
    }

    ll newnum = stoll(to_string(acc) + to_string(nums[ind]));
    return valid(newnum, ind+1, target, nums);
}
