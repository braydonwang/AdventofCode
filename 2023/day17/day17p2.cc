...
int main() {
    ...
    vector<vector<vector<vector<int>>>> dp(r, vector<vector<vector<int>>>(c, vector<vector<int>>(4, vector<int>(11, 1e9))));
    queue<Block> q;
    dp[0][0][0][0] = dp[0][0][1][0] = 0;
    q.push({0,0,0,0}); q.push({0,0,1,0});

    while (!q.empty()) {
       Block cur = q.front(); q.pop();
       int dis = dp[cur.row][cur.col][cur.d][cur.count];
       if (cur.row == r - 1 && cur.col == c - 1 && cur.count >= 4) {
        ans = min(ans,dis);
        continue;
       }
       for (int k = 0; k < 4; k++) {
        if (cur.d != k && cur.count < 4) continue;
        if (cur.d == k && cur.count == 10) continue;
